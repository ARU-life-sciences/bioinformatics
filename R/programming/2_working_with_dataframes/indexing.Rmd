...
useful by itself, but we can index using the results from expressions. Expressions
allow us to both filter and sort the data.
...
not number. We saw this in `reading_and_viewing.Rmd`.
...
vectors of `yield`, so we can index those further. Vectors can only be indexed
with a single number, as they are 1-dimensional. If you try `yield[5, 2]` an error
will throw.
...
Now to do something a bit more complicated. We can combine multiple logical
expressions to filter on multiple conditions. If we wanted to combine the selected
variety and yield over 50 bushels per acre, we could do something like this.
...
```
Sometimes we want to sort our data. If you think about this problem it's a question
of generating a vector of indices which describe how the data should be reordered.
Luckily, R has good functions for achieving this.
...
sorted_yield
```
This vector basically says the first element should be in position 71 in the 
sorted data, the second element in position 113... and so on. Now we can index
our data using this vector.
...
Notice that this sorts in increasing order. If we want the reverse, that's 
surprisingly easy. We use a function called `rev`.
...
Lastly, there are specific functions to find the index of the maximum/minimum 
values. These can sometimes be useful - but all they are doing are returning
_indexes_.
...
2. Get the third column of the data frame by number _and_ by name.
3. Order the data frame by year. First sort the `barley$year` vector, and then
index the whole data frame.
